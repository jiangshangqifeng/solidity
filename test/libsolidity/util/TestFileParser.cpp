...
FunctionValue TestFileParser::parseFunctionCallValue()
{
	try
	{
		...
		if (token != Token::Lat && token != Token::Von)
			BOOST_THROW_EXCEPTION(TestParserError("Invalid value unit provided. Coins can be von or lat."));
	...
	{
		BOOST_THROW_EXCEPTION(TestParserError("Lat value encoding invalid."));
	}
}
...
void TestFileParser::Scanner::scanNextToken()
{
	...
	auto detectKeyword = [](std::string const& _literal = "") -> std::pair<Token, std::string> {
		...
		if (_literal == "lat") return {Token::Lat, ""};
